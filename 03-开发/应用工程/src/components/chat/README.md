...
- `ConfirmationCards.tsx`：渲染阶段 3 通用确认卡的“查看草稿 / 编辑字段 / 返回修改 / 取消 / 确认提交”闭环；旧业务卡只提示重新生成统一草稿，不再提供直接提交入口。
- `confirmation-flow.ts`：阶段 3 通用确认卡的公开字段注册、最终快照、过期判断、服务端不透明字段透传与同步防重复门禁；不展示或自行生成 token、幂等键。
- `OrderAndReturnCards.tsx`：阶段 4 演示身份确认、订单修改/取消申请结果与退换进度时间线；订单写操作仍复用通用确认卡。
- `consumer-error.ts`：把空结果、业务拒绝、超时和系统失败映射为消费者公开状态，隔离服务端内部错误详情。
