...
> 当前状态（2026-09-01）：阶段 3 完成基线为 `main@24aa935`，阶段 4 的兼容契约和模块接口以当前 main 最新提交为准，公共契约仍为 `1.1.0`。阶段 4 准备门禁为 29 个测试文件、160/160 测试与生产构建通过；36 项固定 Evals 为 36/36，指纹 `fp-fc6d604b`。阶段 4 使用 `codex/stage4-*` 全新分支和 worktree；旧 P0、stage3 和 `codex/badcase-*` 分支只保留追溯，不参与本轮合并。详见《阶段4-剩余P0共享架构.md》。
