...
> 当前状态（2026-09-01）：阶段 5 已完成集成验收，公共契约仍为 `1.1.0`。Mock 门禁为 37 个测试文件、256/256 测试与生产构建通过；36 项固定 Evals 为 36/36；真实文字、真实图片 Smoke 与六条 Live 模型 + Mock 业务 E2E 均通过。PCMP / OMS / WMS / TMS / CRM 继续使用 Mock Adapter，真实模型配置仍仅从本机 `.env.local` 读取。详见《阶段5-真实模型Smoke与端到端验收.md》和 `集成报告/2026-09-01-stage5-live-e2e.md`。
...
不建议让每个对话直接修改当前同一个目录。Git 基线、公共契约、独立 branch + worktree 已由 00 建立；后续迭代继续沿用同一治理规则。
