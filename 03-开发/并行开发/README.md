...
> 当前状态（2026-09-01）：阶段 4 已完成，集成基线为 `main@3697d01`，公共契约仍为 `1.1.0`。阶段 5 启动门禁为 36 个测试文件、240/240 测试与生产构建通过；36 项固定 Evals 为 36/36。阶段 5 仅允许真实文字/图片模型替换模型 Adapter，PCMP / OMS / WMS / TMS / CRM 继续使用 Mock Adapter。真实模型测试只能读取本机 `.env.local`，任何对话、提交、Trace 或测试产物都不得包含 API Key。详见《阶段5-真实模型Smoke与端到端验收.md》。
