...
**文档状态**：P0 已完成并通过 Mock 全量回归、36/36 固定 Evals 与真实文字/图片 Smoke
**项目周期**：1～2 周  
...
- 固定评测案例可以重复运行并定位失败环节。
- 所有公开数据为虚拟内容；统一 Qwen3.6 模型网关配置仅保存在本机服务端 `.env.local`。
...
| AC-21 | 密钥与隐私 | 浏览器源码、浏览器直发请求、Trace 和 GitHub 中均不存在模型凭据、图片原始编码或真实个人信息 |
...
P0 已确认并实现：统一 `Qwen3.6-27B` 文字/图片理解网关、Text/Vision 逻辑 Adapter、停止生成、四维路由、统一确认、统一 Trace、本地确定性 RAG、五个页面、统一 Sandbox reset 与 36 项固定 Evals。早期四入口 HTML 原型仅作历史参考，当前消费者端固定三个公开入口，知识问答走隐藏路由。

进入真实企业接入与生产化前仍需确认：

1. PCMP、OMS、WMS、TMS、CRM 的正式职责、主键、字段、权限、更新频率、错误码与 SLA。
2. 模型网关的生产鉴权托管、并发、限流、Token/计费、数据驻留与可公开展示边界。
3. 生产图片上传的病毒扫描、格式/大小/张数限制、保留策略与隐私授权。
4. 正式账号、身份核验、RBAC、知识审核发布、审计归档与多租户隔离。
5. 生产向量检索、知识切片、召回阈值、冲突治理、批量导入与版本回滚。
6. 真实支付、退款、赔偿、判责和上门调度的人工审批与合规边界。

这些事项不影响当前本地 Sandbox 与作品集验收，但在接入真实数据和真实写权限前必须完成设计与安全评审。
